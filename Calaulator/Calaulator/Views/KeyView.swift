...
//  Calaulator
//
//  Created by Apoorv Verma on 13/10/22.
//
...
struct KeyView: View {
        ...
        VStack{
            ...
            HStack {
                RoundedRectangle(cornerRadius: 15)
                    ...
                    .scaleEffect(changeColor ? 1.9 : 1.0)
                ...
                HStack(spacing: 10){
                    ForEach(row, id: \.self){ elem in
                        Button{
                            ...
                            Text(elem.rawValue)
                                .font(.system(size: 30))
                                .frame(width: self.getWidth(elem: elem), height: self.getHeiht(elem: elem))
    ...
    func getHeiht (elem: Keys)-> CGFloat{
    ...
    func didTap(button: Keys){
        ...
        case .decimal, .negative, .percent:
            if button == .decimal{
                ...
                self.value = "\((Double(self.value) ?? 1.0) * (-1.0))"
            }
            else if button == .percent {
                let runningValue = self.runningNumber
                let currentValue = Double(self.value) ?? 0.0
                
                self.value = "\(((runningValue)/100) * currentValue)"
            }
